...
func run(arguments []string) int {
	...
	// Both modes first capture the same isolated, network-free envelope. Probe
	// mode sends only the resulting digest to the authenticated diagnostic
	// service, so a missing credential cannot be misreported as a trace defect.
